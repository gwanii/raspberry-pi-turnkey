...
DEFAUKT_ID_CHARDIGIT = ''.join([string.digits, string.ascii_letters])
DEFAULT_ID_LENGTH = 6
wpa_conf = """country=GB
...
def getssid():
    ssid_list = []
    get_ssid_list = subprocess.check_output(('iw', 'dev', 'wlan0', 'scan',
                                             'ap-force'))
...
def id_generator(size=DEFAULT_ID_LENGTH, chars=DEFAULT_ID_CHARDIGITT):
...
def signin():
    ...
    with open('wpa.conf', 'w') as f:
        f.write(
            wpa_conf.replace('_ssid_', ssid).replace('_password_', password))
    ...
    return render_template(
        'index.html',
        message=
        "Please wait 2 minutes to connect. Then your IP address will show up at <a href='https://snaptext.live/{}'>snaptext.live/{}</a>.".
        format(piid, piid))
...
if __name__ == "__main__":
    ...
    if wificonnected():
        s['status'] = 'connected'
    elif s['status'] == 'connected':  # Don't change if status in status.json is hostapd
        s['status'] = 'disconnected'
    ...
    if s['status'] == 'disconnected':
        ...
        r = requests.post(
            ...
            data=json.dumps({
                "message":
                "Your Pi is online at {}".format(ipaddress),
                "to":
                piid,
                "from":
